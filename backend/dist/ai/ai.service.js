...
const ibm_provider_1 = require("./ibm.provider");
const heuristic_provider_1 = require("./heuristic.provider");
const tools_1 = require("./tools");
const config_1 = require("../config");
class AiService {
    ibmProvider = new ibm_provider_1.IbmWatsonxProvider();
    heuristicProvider = new heuristic_provider_1.HeuristicProvider();
    /**
     * Process a student/faculty query and route through intent classification,
     * tool execution, and structured card assembly.
     */
    async processMessage(userText, context, history) {
        // 1. Intent Classification & Entity Extraction
        const classification = intent_classifier_1.IntentClassifier.classify(userText);
        ...
        switch (intent) {
            case 'EMERGENCY': {
                const kbResult = await tools_1.AiTools.get_campus_information(entities, 'emergency medical health doctor');
            ...
            case 'CAMPUS_NAVIGATION': {
                const locResult = await tools_1.AiTools.find_nearby_locations(entities);
                ...
                if (locations.length > 0) {
                    ...
                    const allLocs = await tools_1.AiTools.find_nearby_locations({});
            ...
            case 'FACILITY_SEARCH': {
                const result = await tools_1.AiTools.find_nearby_locations(entities);
            ...
            case 'LOST_ITEM': {
                const searchResult = await tools_1.AiTools.search_lost_found(entities);
            ...
            case 'EVENT_DISCOVERY': {
                const eventResult = await tools_1.AiTools.search_events(entities);
                ...
                if (events.length > 0) {
                    ...
                    const fallbackEvents = await tools_1.AiTools.search_events({});
            ...
            case 'FOOD_RECOMMENDATION': {
                const foodResult = await tools_1.AiTools.find_food(entities);
            ...
            case 'ROOMMATE_MATCHING': {
                const rmResult = await tools_1.AiTools.find_roommates(context?.userId, entities);
            ...
            case 'CONTACT_ADMIN': {
                const kbResult = await tools_1.AiTools.get_campus_information(entities, 'registrar admin office hours contact');
            ...
            case 'GENERAL_CHAT': {
                try {
                    if (config_1.config.ibmApiKey && config_1.config.ibmProjectId) {
            ...
            default: {
                const infoResult = await tools_1.AiTools.get_campus_information(entities, userText);
