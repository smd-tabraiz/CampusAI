...
class EventsController {
    ...
    static async createEvent(req, res) {
        try {
            ...
            const { title, description, category, date, startTime, endTime, locationName, maxParticipants, isFree, imageUrl, registrationLink } = req.body;
        ...
        try {
            ...
            if (existingRsvp) {
                ...
                if (existingRsvp.status === 'REGISTERED') {
                    await prisma_1.prisma.eventRsvp.delete({
                        where: { id: existingRsvp.id }
                    });
                    res.json({ message: 'RSVP cancelled.', isRegistered: false });
                }
                else {
                    await prisma_1.prisma.eventRsvp.update({
                        where: { id: existingRsvp.id },
                ...
                await prisma_1.prisma.eventRsvp.create({
                    data: {
                        eventId: id,
                        userId: req.user.id,
                        status: 'REGISTERED'
