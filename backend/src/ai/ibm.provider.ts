...
export class IbmWatsonxProvider implements IAiProvider {
  name = 'IBM watsonx.ai / IBM Bob';
  private iamToken: string | null = null;
  private tokenExpiry: number = 0;

  private isConfigured(): boolean {
    return !!(config.ibmApiKey && config.ibmProjectId);
  }
  ...
  async generateResponse(
    ...
    const endpoint = `${config.ibmUrl}/ml/v1/text/generation?version=2023-05-29`;
