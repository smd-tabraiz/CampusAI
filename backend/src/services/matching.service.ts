...
export class MatchingService {
   ...
   */
  static calculateItemMatch(lost: {
    title: string;
    description: string;
    category: string;
    color: string;
   ...
   */
  static calculateRoommateCompatibility(pA: any, pB: any): RoommateCompatibilityResult {
    // 1. Sleep Schedule (25% weight)
    let sleepScore = 70;
    if (pA.sleepSchedule === pB.sleepSchedule) {
      sleepScore = 100;
    } else if (
      (pA.sleepSchedule === 'FLEXIBLE' || pB.sleepSchedule === 'FLEXIBLE')
    ) {
      sleepScore = 88;
    } else {
    ...
    if (pA.cleanliness === pB.cleanliness) {
      ...
      (pA.cleanliness === 'METICULOUS' && pB.cleanliness === 'MODERATE') ||
    ...
    if (budgetDiff === 0) budgetScore = 100;
    else if (budgetDiff <= 1500) budgetScore = 90;
    else if (budgetDiff <= 3000) budgetScore = 75;
    else budgetScore = 60;

    // 5. Lifestyle & Social (15% weight)
    let lifestyleScore = 80;
    if (pA.smokingPreference === pB.smokingPreference) lifestyleScore += 10;
    else lifestyleScore -= 20;
    ...
    if (sleepScore >= 85) matchPoints.push(`compatible ${pA.sleepSchedule.replace('_', ' ').toLowerCase()} sleep schedules`);
    if (cleanScore >= 85) matchPoints.push(`${pA.cleanliness.toLowerCase()} cleanliness style`);
    if (studyScore >= 85) matchPoints.push('aligned study focus');
    if (budgetScore >= 85) matchPoints.push('matching monthly budget');
